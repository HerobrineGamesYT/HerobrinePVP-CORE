				...
				if (args.length == 1) {
					...
					if (target != null) {
						...
						player.sendMessage(ChatColor.GRAY + "You have set the gamemode of " + ChatColor.GOLD
								+ target.getName() + ChatColor.GRAY + " to " + ChatColor.GOLD + "Adventure"
								+ ChatColor.GRAY + ".");
					} else {
					...
					player.sendMessage(ChatColor.GRAY + "Your gamemode has been set to " + ChatColor.GOLD + "Adventure"
							+ ChatColor.GRAY + ".");
				} else {
					player.sendMessage(ChatColor.RED + "Invalid Usage! Usage: /gma OR /gma <player>");
				}
			} else {
