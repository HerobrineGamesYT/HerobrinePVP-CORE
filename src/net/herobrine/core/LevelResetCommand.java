...
public class LevelResetCommand implements CommandExecutor {

    private HerobrinePVPCore main;

    public LevelResetCommand(HerobrinePVPCore main) {
        this.main = main;
    }
        ...
        if (sender instanceof Player) {
            ...
            if (HerobrinePVPCore.getFileManager().getRank(player).getPermLevel() >= 9) {
                ...
                if(!awaitingConfirmation.containsKey(player.getUniqueId())) {
                    ...
                    player.sendMessage(HerobrinePVPCore.translateString("&c&lWARNING: &r&cThis will reset the player levels across the ENTIRE server. Run the command again within 5 seconds if you are sure you want to do this."));
                    confirmTimer(player.getUniqueId());

                }
                else {

                    HerobrinePVPCore.getFileManager().resetAllLevels(player);
