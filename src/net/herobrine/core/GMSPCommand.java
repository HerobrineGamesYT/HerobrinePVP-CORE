			...
			if (HerobrinePVPCore.getFileManager().getRank(player).equals(Ranks.ADMIN)
				...
				if (args.length == 1) {
					...
					if (target != null) {
						...
						player.sendMessage(ChatColor.GRAY + "You have set the gamemode of " + ChatColor.GOLD
								+ target.getName() + ChatColor.GRAY + " to " + ChatColor.GOLD + "Spectator"
								+ ChatColor.GRAY + ".");
					} else {
					...
					player.sendMessage(ChatColor.GRAY + "Your gamemode has been set to " + ChatColor.GOLD + "Spectator"
							+ ChatColor.GRAY + ".");
				} else {
					player.sendMessage(ChatColor.RED + "Invalid Usage! Usage: /gmsp OR /gmsp <player>");
				}
			} else {
